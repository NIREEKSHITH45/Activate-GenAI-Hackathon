...
>**Important**: You can find the Username and Password within the environment by navigating to the **Environment** **(1)** tab in the left pane then copy the **Username** **(2)** and **Password** **(3)**, which will be required for signing into the Azure portal in later steps and make sure to record the **Deployement Id** **(4)** which is may required in next up coming tasks. 
