...
"""
Example usage:
python test_function_calling.py --url "https://llama3-8b-nim-endpoint-aml-1.westeurope.inference.ml.azure.com/v1/chat/completions" \
    --token xxxxxxxxxxx \
    --model "meta/llama-3_1-8b-instruct" \
    --azureml-model-deployment "llama31-8b-instruct-aml-1"
...
Initial Response {
    ...
    "tool_calls": [
        {
            "id": "chatcmpl-tool-f9c2e0f03d334eabab1148dbdac2cccf",
            "type": "function",
            "function": {
...
Follow up Response {
    "role": "assistant",
    "content": "Here is the current weather for San Francisco, CA: 68 degrees Fahrenheit."
}

API reference docs: https://docs.nvidia.com/nim/large-language-models/latest/api-reference.html
"""
...
def run_test(url, token, model="meta/llama-3.1-8b-instruct", azureml_model_deployment=""):
    headers = {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, required=True, default="meta/llama-3_1-8b-instruct", help="The model name to use.")
    parser.add_argument("--url", type=str, required=True, help="The endpoint URL to send the request to.")
    parser.add_argument("--token", type=str, help="API key for the endpoint if any.")
    parser.add_argument("--azureml-model-deployment", type=str, help="Name of the AzureML deployment")

    args = parser.parse_args()

    run_test(args.url, args.token, args.model, args.azureml_model_deployment)